...
## 情况一：只在本地提交了，还没推送（没执行过 `push`）

这是最简单的，在 VSCode 里就能直接撤销，不会影响远程仓库。

1. 打开左侧 **源代码管理** 面板（Ctrl+Shift+G）。
2. 点击右上角的 **`...`** 菜单。
3. 选择 **Commit（提交） → Undo Last Commit（撤销上一次提交）**。

这样会把上次的提交撤回到“暂存”状态，**文件内容还在**，你可以重新修改、重新提交。

## 情况二：已经推送到了 GitHub（执行过 `push`）

一旦推送到远程，就**不要直接使用 `Undo Last Commit`**，因为本地和远程历史已不同步。这时有两条安全的路：

### 方法 1：用 `git revert` 生成一个新提交来“抵消”错误（**推荐，安全**）

此操作会生成一个新的提交，内容与错误提交完全相反，历史记录可追溯，不会影响其他人。
...
> 如果要撤销更早的某一次提交，可以查到那次的 commit id，然后 `git revert <commit-id>`。

### 方法 2：用 `git reset` 强制回退（**谨慎，会修改历史**）

如果你**确定没有别人基于这次提交工作**，或者只是个人分支，可以直接抹掉错误提交。

- 在 VSCode 终端执行：

  ```bash
  # 回退上一次提交，更改保留在工作区（方便修改后重新提交）
...
- 因为历史被重写，推送时需要**强制推送**：
  ...
  （`--force-with-lease` 比 `-f` 更安全，能防止覆盖他人新推送）

> ⚠️ 强制推送会覆盖远程历史，如果别人已经拉取了你的错误提交，会造成冲突。团队项目里尽量用 revert。

------

## 额外情况：推送到了错误的分支

如果只是想撤掉错误分支上的提交，且该分支只有你一个人用，可以在该分支上执行上面的 revert 或 reset + force push。如果错误分支是 `main`/`master`，务必使用 `revert` 以保证安全。
